...
def N(M):
    ...
    #Create model
    a_train,a_test,b_train,b_test=train_test_split(inputscaler(),outputscaler(),test_size=0.2)
    M = 25
    model = Sequential()
    model.add(Dense(M, input_dim=5, activation='relu'))
    model.add(Dense(M, activation='relu'))
    model.add(Dense(1, activation='linear'))
    print(model.summary())
    #Train model
    model.compile(optimizer = 'adam', loss = 'mean_squared_error', metrics=['accuracy'])
    epochs_hist = model.fit(a_train,b_train, epochs=40, batch_size=64, verbose=1, validation_data=(a_test,b_test))
